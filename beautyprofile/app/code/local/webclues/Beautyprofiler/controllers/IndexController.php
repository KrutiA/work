...
class Webclues_Beautyprofiler_IndexController extends Mage_Core_Controller_Front_Action{
    ...
    if(Mage::getSingleton('customer/session')->isLoggedIn()) {
     ...
     public function SaveAction() 
     {
        foreach($this->getTableField() as $key => $val) {
             if (!array_key_exists($key, $_POST)) {
                 $_POST[$key] = $val; // BEFORE $_POST[$key] = 0;
             }
         }
     ...
     if(null !== (Mage::getSingleton('core/session')->getLastinsid()))
      {
        if($this->checkEmail() && $this->checkSelfprofile($customerData->getID(),$_POST['whose_beautyprofile']))
        {
          if($_POST['whose_beautyprofile'] == 'self')
          {
          $_POST['is_default'] = 1;  
          }
      ...
      //  $_POST['profile_status'] = 1;
        /*if($_POST['whose_beautyprofile'] == 'self')
          {
            if($this->checkSelfprofile($customerData->getID(),'profile_type'))
            {
            $_POST['is_default'] = 1;
            }
      ...
      *developer : Kruti Aparnathi
      */
       protected function checkSelfProfile($loginuser_id,$profile_type){
       /* print_r(Mage::app()->getRequest()->getParam('id')); exit;*/
        if($profile_type == 'self')
        {
          $profile = Mage::getModel('beautyprofiler/beutyprofiler')->getCollection()
                    ->addFieldToSelect('*')
                    ->addFieldToFilter('customer_entity_id',$loginuser_id)
                    ->addFieldToFilter('is_default',1)
                    ->load();
                   // echo count($profile->getData());
          if(count($profile->getData()) > 0){
            //echo $profile->getData()[0]['profile_id'].'<br>'; 
            //echo Mage::getSingleton('core/session')->getLastinsid();

            //user enter self profile later
            if((Mage::app()->getRequest()->getParam('id') != null) && ($profile->getData()[0]['profile_id'] != Mage::app()->getRequest()->getParam('id'))) 
            {
              $html['error_profile'] = 'Self Profile Already Exist';
                echo json_encode($html); exit;
            }
            else if((Mage::app()->getRequest()->getParam('id') == null) && ($profile->getData()[0]['profile_id'] != Mage::getSingleton('core/session')->getLastinsid()))
            {
              //echo "else if"; exit;
              $html['error_profile'] = 'Self Profile Already Exist';
                echo json_encode($html); exit;
   ...
   { 
     ...
     //hand and feet data
     if($alldata['hands_and_feet'] == 1){
      $profiledata['hand_and_feet'][] = 'Dry';
     }
     if($alldata['hands_and_feet'] == 2){
      $profiledata['hand_and_feet'][] = 'Regular';
     }
     if($alldata['undertone_warm'] == 2){
      $profiledata['hand_and_feet'][] = 'Warm';
     }
    if($alldata['undertone_neutral'] == 3){
  ...
  */
    ...
    */
    public function SetprofileAction()
    {
      ...
      $opt = Mage::app()->getRequest()->getParam('opt');
...
<table border="0" cellpadding="0" cellspacing="0" width="600px">
 <tr>
    ...
    <tr>
        <td><p style="font-size:13px;padding:0px 15px;color:#000;font-family:Verdana,Arial; margin-bottom:10px;">Hello '.$loggedin_customer_name.', </p></td>
    </tr>
    <tr>
        <td><p style="font-size:13px;padding:0px 15px;color:#000;font-family:Verdana,Arial; margin-bottom:10px; margin-top:0px;">Please! find your attached beauty profile summary.</p></td>
    </tr>
</table>
</td>
</tr>
<tr>
        ...
        $loggedin_customer_email;
        $beauty_profile_entered_email;
        ...
        if (!$mail->send()) { 
          ...
          //$errors = "There was some error sending mail. Please try again later."; 
    ...
    {
      if(Mage::getSingleton('customer/session')->unsNoprofile())
      {
        $json['status'] = 'success';
      }
      echo json_encode($json); exit;
